...
class PixelAccuracy(Metric):
    """Computes the Pixel Accuracy (PA) per class and mean Pixel Accuracy (mPA).

    PA is calculated as the proportion of correctly classified pixels per class.
    mPA is the average of per-class accuracies.
    ...
    def add(self, predicted, target):
        """Adds the predicted and target pair to the Pixel Accuracy metric.
    ...
    def value(self):
        """Computes the PA and mPA.

        Returns:
            Tuple: (PA, mPA). The first output is the per class Pixel Accuracy,
            for K classes it's a numpy.ndarray with K elements. The second output,
