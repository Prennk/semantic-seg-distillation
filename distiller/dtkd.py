...
class DTKD(nn.Module):
    ...
    def forward(self, logits_student, logits_teacher):
        ...
        logits_student_max, _ = torch.nn.functional.softmax(logits_student.max(dim=1, keepdim=True))
        logits_teacher_max, _ = torch.nn.functional.softmax(logits_teacher.max(dim=1, keepdim=True))
